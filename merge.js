...
function randomizeMood() {
  ...
  video.addEventListener("play", () => {
    ...
    function detectEmotionAndRandomizeMood(detections) {
      if (detections.length > 0) {
        ...
        // Check the detected emotion and log it
        if (maxEmotion === "sad") {
          console.log("Emotion detected: Sad");

          // Randomly choose between "benevolent" or "mischievous"
          const mood = Math.random() < 0.5 ? "benevolent" : "mischievous";
          console.log("Chosen Mood for Sad: ", mood);
        }

        if (maxEmotion === "happy") {
          console.log("Emotion detected: Happy");

          // Randomly choose between "benevolent" or "mischievous"
          const mood = Math.random() < 0.5 ? "benevolent" : "mischievous";
          console.log("Chosen Mood for Happy: ", mood);
        }

        if (maxEmotion === "angry") {
          console.log("Emotion detected: Angry");

          // Randomly choose between "benevolent" or "mischievous"
          const mood = Math.random() < 0.5 ? "benevolent" : "mischievous";
          console.log("Chosen Mood for Angry: ", mood);
        }
      }
    }
    setInterval(async () => {
      ...
      if (detections.length > 0) {
        ...
        if (maxEmotion === "disgusted") {
          console.log("Emotion detected: Disgusted");
        }
