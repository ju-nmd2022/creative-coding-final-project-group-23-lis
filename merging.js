...
window.addEventListener("load", () => {
  ...
  //set mood based on the randomized number
    ...
    function generateArt(emotion, mode) {
        if (emotion === "neutral") {
            comment.innerHTML = "You are not displaying any emotion. The artist is waiting for some emotion.";
...
//   }
  ...
  function drawSadNeutralArt() {
    ...
    // Apply angry face overlays for nose, eyes, and mouth
    document.getElementById("angry-image").style.display = "block";
    comment.innerHTML = getRandomNeuComment("angry");

    // Apply random angry face overlays
    const noseOverlay = angryFaceOverlays("nose");
    const eyesOverlay = angryFaceOverlays("eyes");
    const mouthOverlay = angryFaceOverlays("mouth");

    // Display the overlays on top of the video or art
    document.getElementById("nose-round").src = noseOverlay;
    document.getElementById("nose-wings").src = eyesOverlay;
    document.getElementById("lip-wide").src = mouthOverlay;

    document.getElementById("nose-round").style.display = "block";
    document.getElementById("nose-wings").style.display = "block";
    document.getElementById("lip-wide").style.display = "block";
}
  ...
  function angryFaceOverlays(features) {
    // Get the specific feature array from neutralFaceOverlaysAngry based on the passed argument
    const featureArray = neutralFaceOverlaysAngry[features];
  
    // Get a random index from the feature array
    const randomIndex = Math.floor(Math.random() * featureArray.length);
  
    // Return the random selected feature (image path)
    return featureArray[randomIndex];
}
